...
public final class Database implements Serializable {
    ...
    public static TechSupport getTechSupportWorkerById(String id) {
    	TechSupport t = getTechSupportWorkers().stream().filter(x -> x.getId().equals(id)).findFirst().orElse(null);
    	return t;
    }
    
    public static Book getBookById(String name) {
    	Book b = Database.getBooks().stream().filter(x -> x.getName() == name).findFirst().orElse(null);
    	return b;
    }
    
    public static Course getCourseByName(String name) {
    	Course c = Database.getCourses().stream().filter(x -> x.getName().equals(name)).findFirst().orElse(null);
    	return c;
    }
	...
	public static School getSchoolByName(String name) {
		School s = Database.getSchools().stream().filter(x -> x.getFaculty().equals(name)).findFirst().orElse(null);
		return s;
	}
	
	public static Vector<TechSupport> getTechSupportWorkers() {
		Vector <TechSupport> techSupportWorkers = Database.getUsers().stream().filter(x -> x instanceof TechSupport).map(x -> (TechSupport)x).collect(Collectors.toCollection(Vector::new));
